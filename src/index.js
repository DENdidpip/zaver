...
function runUnitTests() {
  ...
  const oldSil = silhouette, oldPieces = pieces;
  try {
    const sq = [{x:10,y:10},{x:90,y:10},{x:90,y:90},{x:10,y:90}];
    assert('point inside', pointInOrOnPoly(50,50,sq));
    assert('point outside', !pointInOrOnPoly(5,5,sq));
    silhouette = sq;
    pieces = [new Piece([{x:10,y:10},{x:90,y:10},{x:90,y:90},{x:10,y:90}], '#00f')];
    const resStrict = computeCoverageAndOverlap(true);
    assert('coverage exact match uncovered==0', resStrict.uncovered === 0);
    assert('coverage exact match overlap==0', resStrict.overlap === 0);
    pieces = [new Piece([{x:20,y:20},{x:100,y:20},{x:100,y:100},{x:20,y:100}], '#00f')];
    const res2 = computeCoverageAndOverlap(true);
    assert('coverage misaligned uncovered>0', res2.uncovered > 0);
  } catch (err) {
    out.push('ERROR: ' + err);
  }
  silhouette = oldSil; pieces = oldPieces; drawAll();
...
function runStressTest() {
  ...
  for (let i = 0; i < reps; i++) { trySnapAll(1,20,4000,false); computeCoverageAndOverlap(false); }
  const t1 = performance.now();
  out.push(`Main-thread sync heavy runs (${reps}) took ${(t1-t0).toFixed(1)}ms`);
  if (!_worker) {
    out.push('Worker not available; skipping worker path.');
    document.getElementById('testResults').textContent = out.join('\n');
    return;
  }
  let seq = Promise.resolve();
  const t2 = performance.now();
  for (let i=0;i<reps;i++) seq = seq.then(()=>workerCheckAsync(false));
  seq.then(() => {
    const t3 = performance.now();
    out.push(`Worker async checks (${reps}) completed in ${(t3-t2).toFixed(1)}ms (wall time)`);
    document.getElementById('testResults').textContent = out.join('\n');
  }).catch(err => {
    out.push('Worker error: ' + err);
    document.getElementById('testResults').textContent = out.join('\n');
  });
  document.getElementById('testResults').textContent = out.join('\n');
}

function forceSnapNow() {
  if (_worker) {
    workerSnapAsync({maxIter:3, snapThreshold:24, opLimit:4000}).then(d => {
      if (d && d.improved) for (let i=0;i<d.pieces.length && i<pieces.length;i++) pieces[i].points = d.pieces[i].points;
      workerCheckAsync(true).then(sr => { coverageOverlay = sr.overlay ? bufferToCanvas(sr.width, sr.height, sr.overlay) : null; drawAll(); });
    });
  } else {
    trySnapAll(3,24,4000,false);
    const r = computeCoverageAndOverlap(true); coverageOverlay = r.overlay; drawAll();
  }
}

const runTestsBtn = document.getElementById('runTestsBtn'); if (runTestsBtn) runTestsBtn.addEventListener('click', runUnitTests);
const stressBtn = document.getElementById('stressBtn'); if (stressBtn) stressBtn.addEventListener('click', runStressTest);
const snapBtn = document.getElementById('snapBtn'); if (snapBtn) snapBtn.addEventListener('click', forceSnapNow);
